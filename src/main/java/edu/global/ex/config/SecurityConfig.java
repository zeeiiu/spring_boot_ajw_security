...
@Configuration /*
				 ...
				 * 객체들이 들어가고 IOC컨테이너가 관리하는 것이 기본 컨셉.
				 */
...
public class SecurityConfig extends WebSecurityConfigurerAdapter { // 무조건 상속해야지 시큐리티를 작동 시키는 파일이 됨. 정해진 것.
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		// .antMatchers("/**").hasAnyRole("ADMIN") ;// 루트로 치고 들어오는 모든 폴도둘운 권한이 어드민인 것만
		// 치고 들어와라. 이제 유저로 치고 가면 안될 것임.
		// .antMatchers("/**").permitAll() // 권한 체크하지 말고 그냥 들어와라. 위에서 부터 체크함. 이거 주석 풀면
		// 바로 위에 ';' 빼야함

		// http.formLogin(); //스프링 시큐리티에 있는 기본 로그인 폼을 사용하겠다. 주석 처리해도 디폴트로 나옴.
		http.formLogin().loginPage("/login") // loginPage()는 말그대로 로그인할 때 페이지 url이고
